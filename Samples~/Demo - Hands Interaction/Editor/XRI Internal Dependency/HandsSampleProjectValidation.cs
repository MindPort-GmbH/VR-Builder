...
namespace VRBuilder.Samples.HandsInteraction.Editor
{
    ...
    public static class HandsSampleProjectValidation
    {
        ...
        static readonly List<BuildValidationRule> s_BuildValidationRules = new List<BuildValidationRule>
        {
            ...
            new BuildValidationRule
            {
                Message = $"[{k_SampleDisplayName}] {k_StarterAssetsSampleName} sample from XR Interaction Toolkit ({k_XRIPackageName}) package must be imported or updated to use this sample. {GetImportSampleVersionMessage(k_XRIPackageDisplayName, k_StarterAssetsSampleName, ProjectValidationUtility.minimumXRIStarterAssetsSampleVersion)}",
                Category = k_Category,
                CheckPredicate = () => ProjectValidationUtility.SampleImportMeetsMinimumVersion(k_XRIPackageDisplayName, k_StarterAssetsSampleName, PackageVersionUtility.GetPackageVersion(k_XRIPackageName)),
                FixIt = () =>
                {
            ...
            new BuildValidationRule
            {
                Message = $"[{k_SampleDisplayName}] {k_HandsInteractionDemoName} sample from XR Interaction Toolkit ({k_XRIPackageName}) package must be imported or updated to use this sample. {GetImportSampleVersionMessage(k_XRIPackageDisplayName, k_HandsInteractionDemoName, ProjectValidationUtility.minimumXRIStarterAssetsSampleVersion)}",
                Category = k_Category,
                CheckPredicate = () => ProjectValidationUtility.SampleImportMeetsMinimumVersion(k_XRIPackageDisplayName, k_HandsInteractionDemoName, PackageVersionUtility.GetPackageVersion(k_XRIPackageName)),
                FixIt = () =>
                {
                    if (TryFindSample(k_XRIPackageName, string.Empty, k_HandsInteractionDemoName, out var sample))
                    {
                        sample.Import(Sample.ImportOptions.OverridePreviousImports);
                    }
                },
                FixItAutomatic = true,
                Error = !ProjectValidationUtility.HasSampleImported(k_XRIPackageDisplayName, k_HandsInteractionDemoName),
            },
            new BuildValidationRule
            {
