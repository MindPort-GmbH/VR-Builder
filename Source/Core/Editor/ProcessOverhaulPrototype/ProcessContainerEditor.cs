...
namespace VRBuilder.Editor.Configuration
{
    ...
    /// </summary>
    [CustomEditor(typeof(ProcessContainer))]
    public class ProcessContainerEditor : UnityEditor.Editor
    {
...
#pragma warning restore 0618
        ...
        {
            container = target as ProcessContainer;
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUI.BeginDisabledGroup(IsProcessListEmpty());
            {
                ...
                GUILayout.BeginHorizontal();
                {
                    ...
                    {
                        GlobalEditorHandler.SetCurrentProcess(ProcessAssetUtils.GetProcessNameFromPath(container.GetSelectedProcess()));
                        GlobalEditorHandler.StartEditingProcess();
                    }

                    if (GUILayout.Button(new GUIContent("Show Process in Explorer...")))
                    {
                        string absolutePath = $"{new FileInfo(ProcessAssetUtils.GetProcessAssetPath(ProcessAssetUtils.GetProcessNameFromPath(container.GetSelectedProcess())))}";
        ...
        private void DrawRuntimeConfigurationDropDown()
        {
            int index = configurationTypes.FindIndex(t =>
                t.AssemblyQualifiedName == container.GetRuntimeConfigurationName());
            index = EditorGUILayout.Popup("Configuration", index, configurationTypeNames);
            container.SetRuntimeConfigurationName(configurationTypes[index].AssemblyQualifiedName);
        }
        ...
        private void DrawProcessSelectionDropDown()
        {
            ...
            string processName = ProcessAssetUtils.GetProcessNameFromPath(container.GetSelectedProcess());
            ...
            if (IsProcessListEmpty() == false && container.GetSelectedProcess() != newProcessStreamingAssetsPath)
            {
        ...
        private void UpdateAvailableProcesses()
        {
            ...
            if (string.IsNullOrEmpty(container.GetSelectedProcess()))
            {
                SetConfiguratorSelectedProcess(ProcessAssetUtils.GetProcessStreamingAssetPath(processDisplayNames[0]));
                GlobalEditorHandler.SetCurrentProcess(ProcessAssetUtils.GetProcessAssetPath(container.GetSelectedProcess()));
            }
        }
    }
}
